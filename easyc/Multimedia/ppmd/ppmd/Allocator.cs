...
namespace PpmdSharp
{
    ...
    /// <item><see cref="PpmState"/></item>
    /// <item><see cref="MemoryNode"/></item>
    /// </list>
    ...
    /// <see cref="AllocatorSize"/> is a field rather than a property).
    /// </remarks>
    internal static class Allocator
    {
        ...
        public static MemoryNode[] MemoryNodes;
        ...
        public static void Initialize()
        {
            ...
            {
                MemoryNodes[index] = new MemoryNode((uint) (NodeOffset + index * MemoryNode.Size));
        ...
        {
            uint size = (uint) allocatorSize << 20;
            if (AllocatorSize != size)
            {
                Stop();
                byte[] memory = new byte[HeapOffset + size];  // the single, large array of bytes
                Pointer.Memory = memory;
                MemoryNode.Memory = memory;
                Model.PpmContext.Memory = memory;
                PpmState.Memory = memory;
                Heap = new Pointer(HeapOffset);  // reserve bytes in the range 0 .. HeapOffset - 1
                AllocatorSize = size;
            }
        }
        ...
        {
            MemoryNode memoryNode = new MemoryNode(LocalOffset);
            memoryNode.Stamp = 0;
